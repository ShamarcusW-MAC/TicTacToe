...
struct GameBoard {
    ...
    func winner() -> (player: GamePlayer, tiles: [GameTile])? {
        ...
        let checkMatch: (_ tile1: GameTile, _ tile2: GameTile, _ tile3: GameTile) -> (player: GamePlayer, tiles: [GameTile])? = { tile1, tile2, tile3 in
        ...
        for y in 0..<3 {
            if let result = checkMatch(grid[y][0], grid[y][1], grid[y][2]) {
                return result
            }
        }
