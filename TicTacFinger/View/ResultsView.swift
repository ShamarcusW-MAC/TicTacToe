...
struct ResultsView: View {
    ...
    func badgeView(contentWidth: CGFloat, contentHeight: CGFloat) -> some View {
        HStack {
            ...
            VStack {
                if winner == .x {
                    winXGameView(contentWidth: contentWidth, contentHeight: contentHeight)
                } else if winner == .o {
                    winOGameView(contentWidth: contentWidth, contentHeight: contentHeight)
                } else {
                    catsGameView(contentWidth: contentWidth, contentHeight: contentHeight)
                }
            }
            Spacer()
        }
...
struct ResultsView_Previews: PreviewProvider {
    static var previews: some View {
        ResultsView(gameResultsText: "Cat's Game", winner: nil, newGameAction: { })
    }
}
